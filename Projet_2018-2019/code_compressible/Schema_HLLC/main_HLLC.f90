PROGRAM CODEHLL
  ...
  real*8:: dt,dx, tinit, tfinale, gamma
...
!  print*, x
  ...
  do while(tinit<tfinale)
     ...
     do i =2,ng+nd
        dt = dx/2.36
        w1(:,i)=w0(:,i)-(dt/dx)*( FHLLC(w0(:,i+1),w0(:,i),gamma)-FHLLC(w0(:,i),w0(:,i-1),gamma))
     end do
     w1(:,size(x))=w0(:,size(x))-(dt/dx)*(FHLLC(w0(:,size(x)),w0(:,size(x)-1),gamma)-FHLLC(w0(:,size(x)-1),w0(:,size(x)-2),gamma))
     w0 = w1
     tinit = tinit + dt
     print*, tinit
  end do

  open(1,file='resultat_HLLC.txt')
  do i=1,nd+ng+1
